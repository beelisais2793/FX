...
namespace Analog::Filters::VAVectorSVF
{
    ...
    {    
        ...
        VecSVF(float Fs, float Fc, float Q) {
            fc = Fc;
            fs = Fs;
            q = Q + 0.5;        
            z1 = z2 = 0.0;
        ...
        SIMD Tick(SIMD I, float A = 1, float X = 1, float Y = 1, float Z = 1, float OA=1, float minC = -1, float maxC = 1)
        {                
            float wd = 2*M_PI*fc;
            float T  = 1/fs;
            float temp = X*wd*T/2;
