/**************************************************************************
    ...
    SCRAWL.CPP - A dumb drawing app demo for Sound Control

    Collects mouse movement data and uses it to control STK 
	PhISEM synthesis, to demonstrate real-time cursor sonification.
...
/**************************************************************************
	...
	Hacked by Perry R. Cook, 1999-2002
...
const char c_szAppName[] = "Drag Sound (Right Click Mouse to Close)";    /* My name */
...
/******************   BAD BOY!!!!  GLOBALS for SKINI ******/
  char hostName[256];
  int theSocket = 0;
  struct sockaddr_in saServer;
		char s[128] = {"//////******  CONNECTION FROM SCRUB!  *******\n"};
  int first = 1;

  PBUTTON0INFO		pb0GLOB = 0;
...
void UpdateCursorPosition(int dx, int dy)
{
	...
	/********************   SKINI MESSAGE TO Synth Engine ********/
	
	if (!first)	{
		if (g_y < DINPUT_CYBITMAP/2) {
			t_total = fabs(g_x - lastX); 
			ynorm = 110.0 - ((double) g_y / 3.0);
			if (ynorm > 127) ynorm = 127;
			if (t_total > DINPUT_CXBITMAP/20)	{
				t_total *= 10;
				if (t_total > 127) t_total = 127;
				lastX = g_x;
				sprintf(s, "ControlChange 0.0 1 1 %f\n",ynorm);
				send(theSocket, s,	strlen(s), 0);
				sprintf(s, "ControlChange 0.0 1 4 100\n");
				send(theSocket, s,	strlen(s), 0);
			...
			t_total = 10 * sqrt((dx*dx) + (dy*dy));
			if (t_total > 127.0) t_total = 127.0;
			sprintf(s, "ControlChange 0.0 1 1 10.0\n");
			send(theSocket, s,	strlen(s), 0);
			xnorm = (g_x*g_x) / 2 /(double) DINPUT_CXBITMAP;
			xnorm += 1;
			if (xnorm > 127) xnorm = 127;
			sprintf(s, "ControlChange 0.0 1 4 %f\n",xnorm);
			send(theSocket, s,	strlen(s), 0);
			sprintf(s,"ChannelPressure\t%.3f  %d  %.1f\n",0.0,0,t_total);
			send(theSocket, s,	strlen(s), 0);
		}
...
void Scrawl_OnButton0Down_FlushMotion(PBUTTON0INFO pb0i)
{
    int i,j;
	double phase = 0;
	if (!pb0GLOB) pb0GLOB = pb0i;
    if (pb0i->fMoved) {
        pb0i->fMoved = 0;
        pb0i->dwSeqLastSeen = 0;
	}

	if (first == 1 && pb0GLOB != 0)	{
		i = 0;
		while (i < DINPUT_CXBITMAP)	{
			MoveToEx(pb0GLOB->hdcWindow, i,   0, NULL); 
			LineTo(pb0GLOB->hdcWindow, i, DINPUT_CYBITMAP/2);
			MoveToEx(g_hdc, i,   0, NULL); 
			LineTo(g_hdc, i, DINPUT_CYBITMAP/2);
			i += (int) (5.0 * fabs(sin(phase += (3.14159268 / 10))));
		}
		for (i=0;i<DINPUT_CXBITMAP;i++)	{
			for (j=DINPUT_CYBITMAP/2;j<DINPUT_CYBITMAP;j++)	{
				if (rand() < i*30)	{
				    MoveToEx(pb0GLOB->hdcWindow, i, j, NULL); 
					LineTo(pb0GLOB->hdcWindow, i, j+1);
					MoveToEx(g_hdc, i,   j, NULL); 
					LineTo(g_hdc, i, j+1);
				}
			}
		}
	    ...
	    sprintf(s,"NoteOn 0.0 1 12 64\n",0.0);
	    send(theSocket, s,	strlen(s), 0);
		Sleep ( (DWORD) 20);
	    sprintf(s, "ControlChange 0.0 1 44 0.0\n");
	    send(theSocket, s,	strlen(s), 0);
		Sleep ( (DWORD) 20);
	    sprintf(s, "ControlChange 0.0 1 11 60.0\n");
	    send(theSocket, s,	strlen(s), 0);
...
HWND AppInit(HINSTANCE hinst, int nCmdShow)
{
    ...
    /*
     *  Get our crosshairs cursor and extract the width and
     *  hotspot location so we can draw it manually.
     */
    g_hcurCross = LoadCursor(NULL, IDC_SIZEALL);
...
/*******  STUFF TO DO EXIT THREAD FROM WINDOZE  "Exit"  *******/
	strcpy(s, "ExitProgram\n");
	send(theSocket, s,	strlen(s), 0);
    Sleep((DWORD) 20);
	send(theSocket, s,	strlen(s), 0);
    Sleep((DWORD) 20);
