...
namespace Std {
    ...
    template<typename T>
    struct StdMatrixView
    {
        std::vector<T> * m;
        size_t row;
        size_t N;

        StdMatrixView(std::vector<T> * x, size_t r, size_t n) {
            m = x;
            N = n;
            row = r;
        }
        StdMatrixView(const StdMatrixView<T> & mat) {
            m = mat.m;
            row = mat.row;
            N = mat.N;
        }
        StdMatrixView& operator = (const StdMatrixView<T> & mat) {
            m = mat.m;
            row = mat.row;
            N = mat.N;
            return *this;
        }
        T __getitem(size_t i) { return (*m)[row*N + i]; }
        void __setitem(size_t i, T val) { (*m)[row*N + i] = val; }
    };
    ...
    struct StdMatrix
    {
        ...
        StdMatrixView<T> __getitem(size_t row) {
            return StdMatrixView<T>(&matrix,row,N);
        }
        void __setitem(size_t row, const StdVector<T> & v) {
    ...
    struct StdStringMap
    {
        ...
        StdString __getitem(const StdString & key) { return map[key]; }
        void __setitem(const StdString & key, const StdString & value) { map[key] = value; }

        const char* __getitem(const char * key) { return map[key].__str__(); }
        void __setitem(const char *key, const char* value) { map[StdString(key)] = StdString(value); }
