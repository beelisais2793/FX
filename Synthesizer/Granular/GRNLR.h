...
public:
    ...
    void process (float * currentBlock, float * fileBuffer, int numChannels, int blockNumSamples, int fileNumSamples, long long int time)
    {
        for(int channel=0; channel<numChannels; ++channel){
            const float gain = envelope(time);
            
            // [1]
            float* channelData = currentBlock;
            const float* fileData = fileBuffer;
