...
inline PolyBLEP::PolyBLEP(double sampleRate)
...
inline void PolyBLEP::setdt(double time) {
    freqInSecondsPerSample = time;
}

inline void PolyBLEP::setFrequency(double freqInHz) {
    setdt(freqInHz / sampleRate);
}

inline void PolyBLEP::setSampleRate(double sampleRate) {
    const double freqInHz = getFreqInHz();
    this->sampleRate = sampleRate;
    setFrequency(freqInHz);
}

inline double PolyBLEP::getFreqInHz() const {
    return freqInSecondsPerSample * sampleRate;
}

inline void PolyBLEP::setPulseWidth(double pulseWidth) {
    this->pulseWidth = pulseWidth;
}

inline void PolyBLEP::sync(double phase) {
    t = phase;
    if (t >= 0) {
        t -= bitwiseOrZero(t);
    } else {
...
inline void PolyBLEP::setWaveform(Waveform waveform) {
    this->waveform = waveform;
}

inline double PolyBLEP::get() const {
...
inline void PolyBLEP::inc() {
    t += freqInSecondsPerSample;
    t -= bitwiseOrZero(t);
}

inline double PolyBLEP::getAndInc() {
    const double sample = get();
    inc();
    return sample;
}

inline double PolyBLEP::sin() const {
    return amplitude * std::sin(TWO_PI * t);
}

inline double PolyBLEP::cos() const {
    return amplitude * std::cos(TWO_PI * t);
}

inline double PolyBLEP::half() const {
    double t2 = t + 0.5;
    t2 -= bitwiseOrZero(t2);
...
inline double PolyBLEP::full() const {
...
inline double PolyBLEP::tri() const {
    double t1 = t + 0.25;
    t1 -= bitwiseOrZero(t1);
...
inline double PolyBLEP::tri2() const {
...
inline double PolyBLEP::trip() const {
...
inline double PolyBLEP::trap() const {
    double y = 4 * t;
    if (y >= 3) {
        y -= 4;
    } else if (y > 1) {
...
inline double PolyBLEP::trap2() const {
...
inline double PolyBLEP::sqr() const {
    double t2 = t + 0.5;
    t2 -= bitwiseOrZero(t2);
...
inline double PolyBLEP::sqr2() const {
...
inline double PolyBLEP::rect() const {
...
inline double PolyBLEP::saw() const {
    double _t = t + 0.5;
    _t -= bitwiseOrZero(_t);
...
inline double PolyBLEP::ramp() const {
    double _t = t;
    _t -= bitwiseOrZero(_t);
