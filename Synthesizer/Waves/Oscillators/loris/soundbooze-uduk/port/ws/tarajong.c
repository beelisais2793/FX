/*
   gcc tarajong.c -o tarajong `pkg-config --libs --cflags jack` -lm -Wall -O3
*/
...
process (jack_nframes_t nframes, void *arg)
{
  ...
  jack_default_audio_sample_t tarajongout_1[len];
  jack_default_audio_sample_t tarajongout_2[len];

  float amount = 0.4;
  float k = 2.0 * amount / (1-amount);

  unsigned int i = 0;
  for (i = 0; i < len; i++) {
    float x1 = input1_in[i];
    float x2 = input2_in[i];
    tarajongout_1[i] = (1+k) * x1 / (1 + k * fabsf(x1));
    tarajongout_2[i] = (1+k) * x2 / (1 + k * fabsf(x2));
  }

  memcpy (output1_out, tarajongout_1, sizeof (jack_default_audio_sample_t) * nframes);
  memcpy (output2_out, tarajongout_2, sizeof (jack_default_audio_sample_t) * nframes);
...
{
  const char *client_name = "tarajong";
