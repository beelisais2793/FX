/*
   gcc bridge.c -o bridge `pkg-config --libs --cflags jack` -lm -Wall -O3
*/
...
process (jack_nframes_t nframes, void *arg)
{
  ...
  /*
  jack_default_audio_sample_t bridgeout_1[len];
  jack_default_audio_sample_t bridgeout_2[len];

  bridge transition ...
  */

  memcpy (output1_out, input1_in, sizeof (jack_default_audio_sample_t) * nframes);
  memcpy (output2_out, input2_in, sizeof (jack_default_audio_sample_t) * nframes);
