...
template<typename T>
PolyBLEP::PolyBLEP<T>(T sampleRate, Waveform waveform, T initialFrequency)
...
template<typename T>
void PolyBLEP<T>::setdt(T time) {
    freqInSecondsPerSample = time;
}

void PolyBLEP::setFrequency(T freqInHz) {
    setdt(freqInHz / sampleRate);
}

void PolyBLEP::setSampleRate(T sampleRate) {
...
template<typename T>
T PolyBLEP<T>::getFreqInHz() const {
    return freqInSecondsPerSample * sampleRate;
}

template<typename T>
void PolyBLEP<T>::setPulseWidth(T pulseWidth) {
    this->pulseWidth = pulseWidth;
}

template<typename T>
void PolyBLEP<T>::sync(T phase) {
    t = phase;
    if (t >= 0) {
        t -= bitwiseOrZero(t);
    } else {
...
template<typename T>
void PolyBLEP<T>::setWaveform(Waveform waveform) {
    this->waveform = waveform;
}

template<typename T>
T PolyBLEP<T>::get() const {
...
template<typename T>
T PolyBLEP<T>::sin() const {
    return amplitude * std::sin(TWO_PI * t);
}

template<typename T>
T PolyBLEP<T>::cos() const {
    return amplitude * std::cos(TWO_PI * t);
}

template<typename T>
T PolyBLEP<T>::half() const {
    T t2 = t + 0.5;
    t2 -= bitwiseOrZero(t2);
