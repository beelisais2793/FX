...
void Microtonal::defaults()
{
    ...
    octave[11].x2   = 1;
    Pname = "12tET";
    Pcomment = "Equal Temperament 12 notes per octave";
...
float Microtonal::getnotefreq(int note, int keyshift) const
{
    ...
    if(keyshift != 0) {
        ...
        rap_keyshift *= powf(octave[octavesize - 1].tuning, float(ksoct));
    }
    ...
    if(Pmappingenabled != 0) {
        ...
        freq *= powf(octave[octavesize - 1].tuning, float(degoct));
...
#define FMCREQ(x) if(!((x < micro.x + 0.0001f) && (x > micro.x - 0.0001f))) \
    ...
    if(this->Pname != micro.Pname)
        return true;
    if(this->Pcomment != micro.Pcomment)
        return true;
...
{
    xml->addparstr("name", (char *) Pname.c_str());
    xml->addparstr("comment", (char *) Pcomment.c_str());
