...
void Bank::setname(unsigned int ninstrument, const string &newname, int newslot)
{
    ...
    if(newslot >= 0)
        ss << std::setw(4) << std::setfill('0') << (newslot + 1) << newname;
    else
        ss << std::setw(4) << std::setfill('0') << (ninstrument + 1) << newname;

    newfilename = dirname + '/' + legalizeFilename(ss.str()) + ".xiz";
...
void Bank::savetoslot(unsigned int ninstrument, Instrument *part)
{
    ...
    std::stringstream ss;
    ss << std::setw(4) << std::setfill('0') << (ninstrument + 1) << part->Pname;

    string filename = dirname + '/' + legalizeFilename(ss.str()) + ".xiz";

    remove(filename.c_str());
    part->saveXML(filename.c_str());
    addtobank(ninstrument, legalizeFilename(ss.str()) + ".xiz", part->Pname);
}
