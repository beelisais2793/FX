...
void OscilGen::spectrumadjust()
{
    ...
    for(int i = 0; i < synth->oscilsize / 2; ++i) {
        ...
        oscilFFTfreqs[i] = std::polar<fftw_real>(double(mag), double(phase));
    }
}
...
void OscilGen::adaptiveharmonic(fft_t *f, float freq)
{
    ...
    for(int i = 0; i < synth->oscilsize / 2 - 2; ++i) {
        ...
        if(high >= (synth->oscilsize / 2 - 2))
            ...
            if(down) {
                f[high] =
                    std::complex<float>(float(f[high].real() + inf[i].real() * (1.0f - low)),
                            float(f[high].imag() + inf[i].imag() * (1.0f - low)));

                f[high + 1] = std::complex<float>(float(f[high + 1].real() + inf[i].real() * low),
                        float(f[high + 1].imag() + inf[i].imag() * low));
            }
            else {
                hc = float(inf[high].real() * (1.0f - low) + inf[high + 1].real() * low);
                hs = float(inf[high].imag() * (1.0f - low) + inf[high + 1].imag() * low);
            }
...
short int OscilGen::get(float *smps, float freqHz, int resonance)
{
    ...
    if((Prand > 64) && (freqHz >= 0.0f) && (!ADvsPAD)) {
        ...
        for(int i = 1; i < nyquist - 1; ++i) //to Nyquist only for AntiAliasing
            outoscilFFTfreqs[i] *=
                std::polar<fftw_real>(1.0, double(rnd * i * RND));
    }
...
FUNC(pulsesine)
{
    if(a < 0.00001f)
        a = 0.00001f;
    x = float(fmod(x, 1) - 0.5f) * expf((a - 0.5f) * logf(128));
    if(x < -0.5f)
        x = -0.5f;
    else
    if(x > 0.5f)
...
FUNC(stretchsine)
{
    x = float(fmod(x + 0.5f, 1)) * 2.0f - 1.0f;
    a = (a - 0.5f) * 4;
    if(a > 0.0f)
        a *= 2;
    a = powf(3.0f, a);
...
FUNC(spike)
{
    ...
    if(x < 0.5f) {
        if(x < (0.5f - (b / 2.0f)))
            ...
            x = (x + (b / 2) - 0.5f) * (2.0f / b); // shift to zero, and expand to range from 0 to 1
            return x * (2.0f / b); // this is the slope: 1 / (b / 2)
        }
    }
    else {
        if(x > (0.5f + (b / 2.0f)))
...
FILTER(lp)
{
    float gain = powf(1.0f - par * par * par * 0.99f, float(i));
...
FILTER(hp1)
{
    float gain = 1.0f - powf(1.0f - par * par, float(i + 1));
