...
// This is needed since templates are not always smart about inheriting super
// members.
#define INHERIT            \
  using Val<T>::val;       \
  using Val<T>::operator=; \
  T operator[](unsigned i) const { return (*this)(); }

template <class T = gam::real>
struct Impulse : public Val<T> {
  INHERIT;
...
struct RCos : public Val<T> {
  INHERIT;

  /// Constructor
  RCos(const T& frq = T(0), const T& amp = T(1)) : val2(0), c1(0) {
    set(frq, amp);
  }
...
struct RSin : public Val<T> {
  ...
  /*	Note: these methods compute parameters directly from coefs, but are
     buggy...
...
template <class T = double>
struct RSin2 : public Val<T> {
  INHERIT;

  /// Constructor
  RSin2(const T& frq = T(0), const T& phs = T(0), const T& dcy = T(1),
        const T& amp = T(1)) {
    set(frq, phs, dcy, amp);
  }
...
template <class T = gam::real>
struct RAddWrap : public RAdd<T> {
  INHERIT;
  RAddWrap(const T& add, const T& val = T(0), const T& max = T(1),
           const T& min = T(0))
...
#define OF1(Ob, fu, a)        \
  template <class T>          \
  inline Ob<T> fu(T v1 = a) { \
    return Ob<T>(v1);         \
  }

//#define OF2(Ob, fu, a, b)
// template<class T, class U> inline Ob<T> fu(T v1=a,U v2=b){ return
// Ob<T>(v1,v2); }

#define OF2(Ob, fu, a, b)               \
  template <class T>                    \
  inline Ob<T> fu(T v1 = a, T v2 = b) { \
    return Ob<T>(v1, v2);               \
  }

#define OF3(Ob, fu, a, b, c)                      \
  template <class T>                              \
  inline Ob<T> fu(T v1 = a, T v2 = b, T v3 = c) { \
    return Ob<T>(v1, v2, v3);                     \
  }
...
class CReson : public Complex<T> {
 public:
  ...
  CReson(const T& frq = T(0), const T& amp = T(1), const T& phs = T(0),
         const T& dec = T(1)) {
    set(frq, amp, phs);
  }
  ...
  template <class U>
  void amp(const Complex<U>& v) {
    (*this)(v.r, v.i);
  }

  /// Set amplitude decay/grow factor after N iterations
  void decay(const T& target, const T& N = T(1)) {
    // NOTE: this handles negative decays, thought better to leave this to
    // frequency component
    // factor(freq(), (1==N) ? target :
    // (pow(scl::abs(target), 1./N)*scl::sgn(target)));
