...
class SoundFileRecordGUI {
 public:
  static void drawRecorderWidget(OutputRecorder *recorder, double frameRate,
    ...
    if (stateMap.find(recorder) == stateMap.end()) {
      ...
      bool *recordButtonPtr = &(stateMap[recorder].recordButton);
      recorder->registerStopRecordCallback([recordButtonPtr](bool timeout) {
        if (timeout) {
          *recordButtonPtr = false;
        }
      });
    }
          ...
          if (!state.overwriteButton) {
            ...
            while (File::exists(filename) && counter < 9999) {
              ...
              filename = filename.substr(0, lastDot) +
                         std::to_string(counter++) + filename.substr(lastDot);
            }
          }

          if (!recorder->start(filename, frameRate, numChannels,
                               ringBufferSize)) {
