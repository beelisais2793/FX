...
struct MyApp : public App {
 public:
  ...
  void onInit() {
    ...
    conv.configure(audioIO().framesPerBuffer(), IRchannels, numFrames,
                   {{0, {0, 1}}});
  }
  ...
  void onSound(AudioIOData &io) override {
    ...
    memcpy(conv.getInputBuffer(0), io.inBuffer(0),
           io.framesPerBuffer() * sizeof(float));
    conv.processBuffer();
    memcpy(io.outBuffer(0), io.inBuffer(0),
           io.framesPerBuffer() * sizeof(float));
    //      memcpy(io.outBuffer(1), conv.getOutputBuffer(1),
    //      io.framesPerBuffer() * sizeof (float));
  }
...
int main() {
  ...
  app.configureAudio(sampleRate, 1024, 2, 2);
  app.audioIO().print();
  app.start();
}
