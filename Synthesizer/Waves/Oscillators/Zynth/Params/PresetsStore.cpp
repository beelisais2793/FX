...
void PresetsStore::copyclipboard(XMLwrapper *xml, const std::string& type)
{
    clipboard.type = type;
    if(clipboard.data != NULL)
        free(clipboard.data);
...
bool PresetsStore::checkclipboardtype(const std::string& type)
{
    //makes LFO's compatible
    if((type.find("Plfo") == std::string::npos) && (clipboard.type.find("Plfo") == std::string::npos))
        return true;
...
void PresetsStore::copypreset(XMLwrapper *xml, const std::string& type, std::string name)
{
