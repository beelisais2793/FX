...
class PresetsStore
{
    public:
        ...
        //Clipboard stuff
        void copyclipboard(XMLwrapper *xml, const std::string& type);
        bool pasteclipboard(XMLwrapper *xml);
        bool checkclipboardtype(const std::string& type);

        //presets stuff
        void copypreset(XMLwrapper *xml, const std::string& type, std::string name);
