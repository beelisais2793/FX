...
RungeKutta_double_vecvecComplex::RungeKutta_double_vecvecComplex(
    chimera::ChimeraContext* context,
    chimera::EntryPoint* init,
    chimera::simulation::TemplateOdeSystem<double, vec_vec_complex> * system,
    double dt
):
    _context(context),
    _init((size_t)init)
{
    ...
    _state = new vec_vec_complex(length);
...
size_t RungeKutta_double_vecvecComplex::getTimeType() const
{
    return chimera::systemtypes::PID_NUMBER;
}

size_t RungeKutta_double_vecvecComplex::getStateType() const
{
    ...
    return _context->getParameterID(vectorVectorComplexMetaName);
}

//double * RungeKutta::getTime()
void * RungeKutta_double_vecvecComplex::currentTime()
{
...
void RungeKutta_double_vecvecComplex::initialize(chimera::vec_t_LuaItem args)
{
    if(args.size() > 1) {
        if(args[0].getType() == chimera::systemtypes::PID_NUMBER)
        {
            ...
            /*if(args[1].getType() == getStateType())
            {
                ...
                ublas::vector<double>* newState = (ublas::vector<std::complex<double> >*)vd->value;
...
void RungeKutta_double_vecvecComplex::start(chimera::vec_t_LuaItem args)
{
...
bool RungeKutta_double_vecvecComplex::finished() const
{
...
vec_vec_complex operator+(const vec_vec_complex &a, const vec_vec_complex &b)
{
    vec_vec_complex nv(a);
