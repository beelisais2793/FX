...
template<typename T>
class BlitSaw: public Generator<T>
{
 public:
  //! Class constructor.
  BlitSaw( T frequency = 220.0 );
...
template<typename T>
inline StkFrames<T>& BlitSaw<T>::tick( StkFrames<T>& frames, unsigned int channel )
{
...
template<typename T>
void BlitSaw<T>::setHarmonics( unsigned int nHarmonics )
{
...
template<typename T>
void BlitSaw<T>::updateHarmonics( void )
{
  if ( nHarmonics_ <= 0 ) {
