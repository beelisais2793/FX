...
class FileWrite : public Stk
{
  ...
  */
  ...
  //! Write sample frames from the StkFrames<T> object to the file.
  /*!
    An StkError will be thrown if the number of channels in the
    StkFrames<T> argument does not agree with the number of channels
    specified when opening the file.
   */
  void write( StkFrames<T>& buffer );
...
template<typename T>
bool FileWrite<T>::setRawFile( std::string fileName )
{
...
template<typename T>
bool FileWrite<T>::setWavFile( std::string fileName )
{
...
template<typename T>
void FileWrite<T>::closeWavFile( void )
{
...
template<typename T>
void FileWrite<T>::closeSndFile( void )
{
  int bytesPerSample = 1;
  if ( dataType_ == STK_SINT16 )
    bytesPerSample = 2;
...
template<typename T>
bool FileWrite<T>::setAifFile( std::string fileName )
{
  std::string name( fileName );
...
template<typename T>
void FileWrite<T>::closeAifFile( void )
{
...
template<typename T>
bool FileWrite<T>::setMatFile( std::string fileName )
{
...
template<typename T>
void FileWrite<T>::closeMatFile( void )
{
...
void FileWrite<T>::write( StkFrames<T>& buffer )
{
  ...
  if ( buffer.channels() != channels_ ) {
    oStream_ << "FileWrite::write(): number of channels in the StkFrames<T> argument does not match that specified to open() function!";
    handleError( StkError::FUNCTION_ARGUMENT );
    return;
  }
