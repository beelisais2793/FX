...
using std::vector;
template<typename T> using sample_vector = std::vector<T>;
template<typename T> using sample_matrix = sample_vector<sample_vector<T>>;
...
{    
    ...
    template<typename V, typename T>
    sample_vector<sample_vector<T>> deinterleave(size_t n, size_t channels, const V& in) {
        sample_vector<sample_vector<T>> r(n);
        for(size_t i = 0; i < channels; i++)
        {
            r[i].resize(n);
