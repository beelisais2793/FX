...
void DX7::onMidi(uint8_t status, uint8_t data1, uint8_t data2)
{
	uint8_t msg[3] = { status, data1, data2 };
	ring_buffer_.Write(msg, 3);
}

void DX7::onSysex(uint8_t* msg, uint32_t size)
{
	if (size == 4104)
...
{
	synth_unit_->onPatch((uint8_t*)patch, size);
}
...
void DX7::onProcess(size_t n, float * buffer)
{
