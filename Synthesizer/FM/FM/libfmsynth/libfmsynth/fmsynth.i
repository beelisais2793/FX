...
%inline %{
    ...
    {        
        ...
        FMPreset(size_t size = fmsynth_preset_size) {
        ...
        uint8_t __getitem(size_t i) { return buffer[i]; }
        void    __setitem(size_t i, uint8_t val) { buffer = val; }
    ...
    struct FMSynth
    {
        ...
        void set_parameter(unsigned parameter, unsigned index) {
            fmsynth_set_parameter(fm,parameter,index);
        }
        ...
        float convert_from_normalized_parameter(unsigned parameter, float val) {
            return fmsynth_convert_from_normalized_parameter(fm,paramter,val);
        }
        float convert_to_normalized_parameter(unsigned parameter, float val) {
            return fmsynth_convert_to_normalized_parameter(fm,paramter,val);
        }
        ...
        float convert_from_normalized_global_parameter(unsigned parameter, float val) {
            return fmsynth_convert_from_normalized_global_parameter(fm,paramter,val);
        }
        float convert_to_normalized_global_parameter(unsigned parameter, float val) {
            return fmsynth_convert_to_normalized_global_parameter(fm,paramter,val);
        }
        ...
        void set_mod_wheel(uint8_t wheel) { fmsynth_set_modwheel(fm,wheel); }
        void set_pitch_bend(uin16_t val) { fmsynth_set_pitch_bend(fm,val); }
