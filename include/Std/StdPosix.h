...
namespace Std 
{
    ...
    inline float random(float min = 0.0f, float max = 1.0f) { return min + (max-min)* ((float)rand() / (float)RAND_MAX); }
    inline int system(const char * cmd) { return std::system(cmd); }
    inline void abort() { std::abort(); }
    inline void exit(int exit_code=-1) { std::exit(exit_code); }
    inline char* getenv(const char * var) { return std::getenv(var); }
    inline int   setenv(const char * name, const char * val, int overwrite) { return ::setenv(name,val,overwrite); }
    inline int unsetenv(const char * name) { return ::unsetenv(name); }
    inline int putenv(char * string) { return ::putenv(string); }
    inline int raise(int sig) { return std::raise(sig); }
    */
    /*
    struct JumpBuffer 
    {
    ...
    */
    ...
    class ipaddr
    {
        ...
        public:
            ...
            friend std::istream& operator >> (std::istream& lhs, ipaddr& rhs);
            int m_family;
            union
            {
                in_addr v4;
