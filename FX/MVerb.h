...
public:
    ...
    void process(T **inputs, T **outputs, int sampleFrames){
        ...
        if(on_button == false) {
            ...
            memcpy(outputs[1],inputs[1],sampleFrames * sizeof(T));
            return;
        }
    ...
    void randomize() {
        ...
        setParameter(PREDELAY,noise.rand() * 100 * (SampleRate / 1000));
        setParameter(SIZE,noise.rand());
        setParameter(GAIN,noise.rand());
        setParameter(DECAY,noise.rand());
        setParameter(MIX,noise.rand());
        setParameter(EARLYMIX,noise.rand());
    }
    ...
    DspFloatType getParameter(int index){
        switch(index){
            case DAMPINGFREQ:
