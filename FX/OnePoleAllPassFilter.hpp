...
public:
	...
	void processSamples (DspFloatType* const samples,
						 const int numSamples) noexcept;
	
	// Process a single sample.
    // Less efficient method but leaves the sample unchanged,
    // returning a filtered copy of it.
    DspFloatType processSingleSampleRaw (const DspFloatType sampleToProcess) noexcept;
	
	// Set the centre frequency of the allpass with respect to the sample rate
	void makeAllpass (const DspFloatType inverseSampleRate,
                      const DspFloatType centreFrequency) noexcept;
    
private:
    //==============================================================================
	//CriticalSection lock;
	DspFloatType x1, y1, b0, b1, a1;
...
inline DspFloatType OnePoleAllpassFilter::processSingleSampleRaw (const DspFloatType sampleToProcess) noexcept
{
...
void OnePoleAllpassFilter::processSamples (DspFloatType* const samples,
                                    const int numSamples) noexcept
{
...
// calculation.
inline void OnePoleAllpassFilter::makeAllpass (const DspFloatType inverseSampleRate,
                                        const DspFloatType centreFrequency) noexcept
{
    ...
    const DspFloatType w0 = fmin(centreFrequency * inverseSampleRate, 0.99*M_PI);
    const DspFloatType tan_half_w0 = tan(0.5 * w0);
    
    //const ScopedLock sl (lock);
    
    b0 = a1 = (DspFloatType)((1.0 - tan_half_w0) / (1.0 + tan_half_w0));
    b1 = -1.0f;
}
